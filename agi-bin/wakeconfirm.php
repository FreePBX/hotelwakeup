...
<?php
{
	...
	while ( !feof($stdin) ) 
	{
		...
		if ($parm_debug_on) fputs( $stdlog, $temp );
...
/***** removed  LD
		...
		if ($parm_debug_on) fputs( $stdlog, "callerid: $callerid\n\n" );
...
************ end removed ***/
	...
	if ( !$rc[result] ) $rc = execute_agi( "STREAM FILE this-is-yr-wakeup-call \"\" ");
	...
	while ( !$rc[result] && $lgcount < 5)   // set hard limit of 5 repeats
	{
		if ( !$rc[result] ) $rc = execute_agi( "STREAM FILE to-cancel-wakeup \"1234\" ");
		if ( !$rc[result] ) $rc = execute_agi( "STREAM FILE press-1 \"1234\" ");
		if ( !$rc[result] ) $rc = execute_agi( "STREAM FILE to-snooze-for \"1234\" ");
		if ( !$rc[result] ) $rc = execute_agi( "STREAM FILE digits/5 \"1234\" ");
		if ( !$rc[result] ) $rc = execute_agi( "STREAM FILE minutes \"1234\" ");
		if ( !$rc[result] ) $rc = execute_agi( "STREAM FILE press-2 \"1234\" ");
		if ( !$rc[result] ) $rc = execute_agi( "STREAM FILE to-snooze-for \"1234\" ");
		if ( !$rc[result] ) $rc = execute_agi( "STREAM FILE digits/10 \"1234\" ");
		if ( !$rc[result] ) $rc = execute_agi( "STREAM FILE minutes \"1234\" ");
		if ( !$rc[result] ) $rc = execute_agi( "STREAM FILE press-3 \"1234\" ");
		if ( !$rc[result] ) $rc = execute_agi( "STREAM FILE to-snooze-for \"1234\" ");
		if ( !$rc[result] ) $rc = execute_agi( "STREAM FILE digits/15 \"1234\" ");
		if ( !$rc[result] ) $rc = execute_agi( "STREAM FILE minutes \"1234\" ");
	...
	case '49':	
        ...
        execute_agi( "EXEC background \"wakeup-call-cancelled\" ");
				...
				execute_agi( "HANGUP" );
				# Option 1 and just hanging up results in the call being cancelled
...
{
	GLOBAL $parm_chan_ext, $parm_temp_dir, $parm_call_dir, $parm_debug_on, $chan, $sta, $cidn, $agivar, $parm_maxretries, $parm_retrytime, $parm_waittime, $parm_wakeupcallerid, $parm_application, $parm_data, $stdin, $stdout, $stdlog, $date;

#	$w = getdate( $time_wakeup );   # LD Does not seem to be needed
